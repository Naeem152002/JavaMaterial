...
public class UserServiceImpl implements UserService{
	...
	public UserDto saveUser(UserDto userDto) {
		...
		Rating[] ratingsOfUser=restTemplate.getForObject("http://localhost:8086/api/ratings/users/"+user.getUserId(),Rating[].class);
	     logger.info("{}",ratingsOfUser);
	     List<Rating>ratings=Arrays.stream(ratingsOfUser).toList();
	     List<Rating>newRatingList=ratings.stream().map(rating -> {
	    		
	    		ResponseEntity<Hotel> forEntity=restTemplate.getForEntity("http://localhost:8085/api/hotels/"+rating.getHotelId(), Hotel.class);
	...
	public UserDto updateUser(UserDto userDto, int userId) {
		...
		Rating[] ratingsOfUser=restTemplate.getForObject("http://localhost:8086/api/ratings/users/"+u.getUserId(),Rating[].class);
	     logger.info("{}",ratingsOfUser);
	     List<Rating>ratings=Arrays.stream(ratingsOfUser).toList();
	     List<Rating>newRatingList=ratings.stream().map(rating -> {
